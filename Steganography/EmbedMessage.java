//EmbedMessage.java
 ...
 JButton open = new JButton("Cari"), embed = new JButton("Sisipkan"),
    save = new JButton("Simpan di"), reset = new JButton("Reset");
 ...
 public EmbedMessage() {
    super("Form Penyisipan Pesan Modifikasi Metode LSB dengan RPE");
    assembleInterface();
...
//       getMaximumWindowBounds());
 ...
 private void assembleInterface() {
   ...
   save.addActionListener(this);   
    ...
    p.setBorder(BorderFactory.createTitledBorder("Masukkan pesan rahasia:"));
 ...
 private void embedMessage(BufferedImage img, String mess) {
    ...
    if(messageLength * 8 + 32 > imageSize) {
       JOptionPane.showMessageDialog(this, "Pesan terlalu panjang untuk dimasukkan ke dalam gambar ini",
          "Pesan terlalu panjang!", JOptionPane.ERROR_MESSAGE);
       return;
       }
 ...
 private void embedInteger(BufferedImage img, int n, int start, int storageBit) {
    ...
    for(int i=startX; i< ( maxX /2 ) - 1 && count<32; i++) {
 ...
 private void embedByte(BufferedImage img, byte b, int start, int storageBit) {
    int maxX = img.getWidth(), maxY = img.getHeight(), 
       startX = start/maxY, startY = start - startX*maxY, count=0;
    for(int i=startX; i<( maxX /2 ) - 1 && count<8; i++) {
 ...
 private void saveImage() {
    if(embeddedImage == null) {
       JOptionPane.showMessageDialog(this, "Periksa Kembali Inputan Pesan atau Gambar yang dipilih!", 
         "Tidak ada yang tersimpan", JOptionPane.ERROR_MESSAGE);
       return;
      }
   java.io.File f = showFileDialog(false);
    ...
    {
        ext = "bmp";
        f = new java.io.File(f.getAbsolutePath()+".bmp");
    }
